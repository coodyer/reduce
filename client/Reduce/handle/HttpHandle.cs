    ...
    class HttpHandle
    {
        ...
        public HttpResult BaseConn(String url, String method, String data, String encode, Dictionary<String, String> headers)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                {
                    result = httpsSendData(host, port, timeOut, encode, bytes);
                }
                else { result = httpSendData(host, port, timeOut, encode, bytes); }
                if (result == null ||result.Code==-1 || result.Code == 502 || result.Code == 503)
                {
                    continue;
                }
                return result;
